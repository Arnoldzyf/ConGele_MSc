...
def train(train_loader, cVAE, disentangle, optimizer, beta, gamma, batch_size, logging_interval):
    ...
    for batch, (X_tg, X_bg) in enumerate(train_loader):
        ...
        ''' 
        Compute prediction error (sum of a whole batch) 
        '''
        # make prediction
        pred = cVAE(X_tg, X_bg)

        # square error:
        reconst_loss_tg = ((pred["reconst_tg"] - X_tg) ** 2).sum()
        reconst_loss_bg = ((pred["reconst_bg"] - X_bg) ** 2).sum()
        reconst_loss = reconst_loss_tg + reconst_loss_bg

        # KL( p || N(0,1) )
        KL_s_tg = (- 0.5 * (1 + pred["s_lv_tg"] - torch.exp(pred["s_lv_tg"]) - torch.square(pred["s_mu_tg"]))).sum()
        KL_z_tg = (- 0.5 * (1 + pred["z_lv_tg"] - torch.exp(pred["z_lv_tg"]) - torch.square(pred["z_mu_tg"]))).sum()
        KL_z_bg = (- 0.5 * (1 + pred["z_lv_bg"] - torch.exp(pred["z_lv_bg"]) - torch.square(pred["z_mu_bg"]))).sum()
        KL_loss = KL_s_tg + KL_z_tg + KL_z_bg

        # forcing independence
        TC_loss = 0
        discriminator_loss = 0
        if disentangle and current_batch_size != 1:
            if pred["v_score"] is None or pred["v_bar_score"] is None:
                print("Oops! None value returned by the discriminator ------------")
            else:
                # min v_score --> 0
                TC_loss = (torch.log(pred["v_score"]) - torch.log(1-pred["v_score"])).sum()
                # max v_score --> 1, min v_bar_score --> 0
                discriminator_loss = (- torch.log(pred["v_score"]) - torch.log(1 - pred["v_bar_score"])).sum()
